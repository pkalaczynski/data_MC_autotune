...
# CONFIG
# ==========================================
# Run this script from the julia/ directory, same as compare_engines.jl.
# Points at the HiggsML (8D, real dataset) results instead of the
# synthetic (12D) ones -- different files, different n_trials budget.
const PYTHON_RESULTS_CSV = "python_code/output/higgsml_results.csv"
const JULIA_RESULTS_CSV = "output/higgsml_julia_results.csv"
const N_TRIALS_FILTER = 500   # HiggsML uses 500 trials/repeat, vs 150 for the synthetic task

# ==========================================
# 1. MINIMAL CSV LOADER (Base only -- no CSV.jl dependency)
# ==========================================
# Identical to compare_engines.jl's loader -- duplicated rather than shared
# so this script stays self-contained and changes here can't regress the
# already-working synthetic comparison.
...
# ==========================================
println("Loading per-repeat HiggsML results...")

if !isfile(JULIA_RESULTS_CSV)
    error("$JULIA_RESULTS_CSV not found. Run higgsml_multiseed.jl first (in this same directory).")
end
if !isfile(PYTHON_RESULTS_CSV)
    error("$PYTHON_RESULTS_CSV not found. Run `python-code higgsml --n-repeats N` first, or fix the path above.")
end
...
# ==========================================
println("\n" * "="^70)
println("ENGINE COMPARISON -- HiggsML 8D -- $(N_TRIALS_FILTER) trials per run")
println("="^70)
...
ax_loss = Axis(fig[1, 1],
    title="Best Loss (χ²) across repeats -- HiggsML 8D",
...
ax_time = Axis(fig[1, 2],
    title="Wall Time across repeats -- HiggsML 8D",
    ylabel="Seconds",
    xticks=(1:n_groups, labels),
    xgridvisible=false, ygridvisible=true,
    ygridstyle=:dash, ygridcolor=(:gray, 0.25)
...
save("output/higgsml_engine_comparison.png", fig, px_per_unit=2)
save("output/higgsml_engine_comparison.pdf", fig)
println("Saved comparison figure to 'output/higgsml_engine_comparison.png' and '.pdf'")
