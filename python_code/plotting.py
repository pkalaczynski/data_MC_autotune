...
from data_MC_autotune.python_code.config import ENERGY_BINS, CUT_ORDER
...
def _apply_cuts(df, cuts_named):
    c = cuts_named
    return df[
        (df.track_chi2 >= c["c_chi2_l"]) & (df.track_chi2 <= c["c_chi2_h"])
        & (df.isolation >= c["c_iso_l"]) & (df.isolation <= c["c_iso_h"])
        & (df.hits_count >= c["c_hits_l"]) & (df.hits_count <= c["c_hits_h"])
        & (df.timing_ns >= c["c_time_l"]) & (df.timing_ns <= c["c_time_h"])
        & (df.hadronic_fraction >= c["c_had_l"]) & (df.hadronic_fraction <= c["c_had_h"])
        & (df.vertex_dist >= c["c_vtx_l"]) & (df.vertex_dist <= c["c_vtx_h"])
    ]


def plot_before_after(df_data, df_mc, cuts_named, out_path="energy_distribution_comparison.png"):
    ...
    gs = fig.add_gridspec(2, 3, width_ratios=[1.0, 0.95, 0.55], height_ratios=[3, 1],
    ...
    ax1_main.step(centers, h_data_before, where="mid", color="black", lw=2, label="Data")
    ax1_main.step(centers, h_mc_before, where="mid", color="red", lw=2, ls="--", label="MC Raw")
    ...
    ax2_main.step(centers, h_mc_after, where="mid", color="dodgerblue", lw=2, ls="--", label="MC Tuned")
    ...
    handles = ax1_main.get_legend_handles_labels()[0] + [ax2_main.get_legend_handles_labels()[0][-1]]
    ...
    ax_side.text(0.0, 0.55, cuts_text, family="monospace", fontsize=9.5, va="top", ha="left")
