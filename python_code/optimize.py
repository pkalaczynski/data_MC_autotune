...
from config import (
    SEARCH_SPACE, CUT_ORDER, N_TRIALS, RANDOM_SAMPLER_SEED,
    N_DATA_EVENTS, N_MC_EVENTS, DATA_SEED, MC_SEED,
)
from synthetic_data import generate_synthetic_data
from loss import compute_agreement_loss

optuna.logging.set_verbosity(optuna.logging.WARNING)  # keep stdout clean like the Julia script
...
def suggest_cuts(trial: optuna.Trial):
    ...
    for spec in SEARCH_SPACE:
        if spec.is_int:
            values[spec.name] = trial.suggest_int(spec.name, int(spec.low), int(spec.high), step=int(spec.step))
        else:
...
def run(sampler_name: str = "random", n_trials: int = N_TRIALS, verbose: bool = True,
    ...
    if verbose:
        print(f"Running {n_trials}-trial 12D tuning loop via Optuna ({sampler_name} sampler)...")

    t_start = time.perf_counter()
    study.optimize(make_objective(df_data, df_mc), n_trials=n_trials, show_progress_bar=False)
    ...
    if verbose:
        ...
        print("=" * 40)
        print(f"  track_chi2        : [{cuts_named['c_chi2_l']:.2f}, {cuts_named['c_chi2_h']:.2f}]")
        print(f"  isolation         : [{cuts_named['c_iso_l']:.2f}, {cuts_named['c_iso_h']:.2f}]")
        print(f"  hits_count        : [{int(cuts_named['c_hits_l'])}, {int(cuts_named['c_hits_h'])}]")
        print(f"  timing_ns         : [{cuts_named['c_time_l']:.2f}, {cuts_named['c_time_h']:.2f}]")
        print(f"  hadronic_fraction : [{cuts_named['c_had_l']:.2f}, {cuts_named['c_had_h']:.2f}]")
        print(f"  vertex_dist       : [{cuts_named['c_vtx_l']:.2f}, {cuts_named['c_vtx_h']:.2f}]")
        print("=" * 40)
    ...
    with open(f"results_{sampler_name}.json", "w") as f:
