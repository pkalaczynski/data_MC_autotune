...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="python-code", description="Optuna backend for the Data/MC autotune task"
    )
    sub = parser.add_subparsers(dest="command", required=True)

    p_run = sub.add_parser(
        "run",
        help="Single optimization run against Julia's data + Python's own diagnostic plot",
    )
    ...
    p_multi = sub.add_parser(
        "multi-seed", help="Repeat the run across several sampler seeds, log to CSV"
    )
    ...
    if args.command == "run":
        result, df_data, df_mc = run_single(
            sampler_name=args.sampler, n_trials=args.n_trials
        )
