...
from data_MC_autotune.python_code.config import ENERGY_BINS, SIGNAL_MIN, SIGNAL_MAX


def compute_agreement_loss(df_data: pd.DataFrame, df_mc: pd.DataFrame, cuts) -> float:
    (chi2_low, chi2_high, iso_low, iso_high, hits_low, hits_high,
     time_low, time_high, had_low, had_high, vtx_low, vtx_high) = cuts

    def window_mask(df):
        return (
            (df.track_chi2 >= chi2_low) & (df.track_chi2 <= chi2_high)
            & (df.isolation >= iso_low) & (df.isolation <= iso_high)
            & (df.hits_count >= hits_low) & (df.hits_count <= hits_high)
            & (df.timing_ns >= time_low) & (df.timing_ns <= time_high)
            & (df.hadronic_fraction >= had_low) & (df.hadronic_fraction <= had_high)
            & (df.vertex_dist >= vtx_low) & (df.vertex_dist <= vtx_high)
        )
    ...
    # --- Peak protection -------------------------------------------------
    raw_peak_count = ((df_data.energy >= SIGNAL_MIN) & (df_data.energy <= SIGNAL_MAX)).sum()
